...
from cudaqbitcodesimulation import cudaq_simulation, noisy_cudaq_simulation
...
def run_benchmark(benchmark: CudaQBenchmark, noise_levels=[0.0, 0.005, 0.02]):
    ...
    print("hi2")
    # Draw circuit
    print(cudaq.draw(kernel, benchmark.num_data_qubits, benchmark.num_rounds, benchmark.bit_state))
    print(f"\n=== Running Benchmark: {benchmark.__class__.__name__} ===")
    print(f"Data qubits      : {benchmark.num_data_qubits}")
    print(f"Bit state        : {benchmark.bit_state}")
    ...
    # ------------------------------------------------------------
    # Ideal simulation
    ideal_counts_raw = cudaq_simulation(kernel, benchmark.num_data_qubits, benchmark.num_rounds, benchmark.bit_state, shots=100)#2000)
    ...
    for p in noise_levels:
        ...
        noisy_counts_raw = noisy_cudaq_simulation(kernel, benchmark.num_data_qubits, benchmark.num_rounds, benchmark.bit_state, p=p, shots=100)#2000)
...
if __name__ == "__main__":
    ...
    for num_data_qubits in [3, 4]:
        bit_state = [i % 2 for i in range(num_data_qubits)]
        print(f"Running benchmarks for {num_data_qubits} data qubits with bit state {bit_state}...")

        for num_rounds in [1, 2]:
            print(f"\n--- Benchmark with {num_rounds} measurement rounds ---")
            benchmark = BitCode_CUDAQ(num_data_qubits=num_data_qubits,
